...
export default function MobileRemote() {
    ...
    const handleAction = (action: string, payload?: any) => {
        ...
        // Attach freeball status if active and it's a potting action
        let actionPayload = payload ? { ...payload } : {};
        if (isFreeballMode && (action === 'POT_RED' || action === 'POT_COLOR')) {
            actionPayload.isFreeball = true;
            setIsFreeballMode(false); // auto-reset after shot
        }
        
        sendAction(action, Object.keys(actionPayload).length > 0 ? actionPayload : undefined);
    };
    ...
    return (
        <div className="mobile-app dark-theme">
            ...
            <div className="bottom-actions-grid">
                <button 
                    className={`btn-action btn-freeball ${isFreeballMode ? 'active' : ''}`} 
                    onClick={() => setIsFreeballMode(!isFreeballMode)}
                    style={{ background: isFreeballMode ? '#e67e22' : 'rgba(255, 255, 255, 0.1)', color: isFreeballMode ? '#fff' : 'rgba(255, 255, 255, 0.7)' }}
                >
                ...
                <button className="btn-action btn-endframe" onClick={() => {
                    if (!gameState) return;
                    if (!window.confirm("Voulez-vous vraiment terminer cette frame ?")) return;
