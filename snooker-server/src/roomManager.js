...
const persistTimeoutIds = new Map();
...
async function persistGameState(roomId, gameState) {
    ...
    if (persistTimeoutIds.has(roomId)) {
        clearTimeout(persistTimeoutIds.get(roomId));
    }

    persistTimeoutIds.set(roomId, setTimeout(async () => {
        try {
            await db.collection('rooms').doc(roomId).set({
                gameState: room ? room.gameState : gameState,
                lastUpdated: new Date().toISOString()
            });
        } catch (err) {
            console.error(`Failed to persist room ${roomId}:`, err);
        } finally {
            persistTimeoutIds.delete(roomId);
        }
    }, 1500));
}
