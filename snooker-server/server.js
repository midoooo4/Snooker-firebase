...
function loadConfig() {
    ...
    return { activeTablesCount: 1, pricePerFrame: 20, appTheme: 'emerald' };
}

function saveConfig() {
    try {
        fs.writeFileSync(CONFIG_FILE, JSON.stringify({ activeTablesCount, pricePerFrame, appTheme }, null, 2));
    } catch (e) {
...
let appTheme = savedConfig.appTheme || 'emerald';
...
app.post('/api/admin/config/tables', verifyAdminToken, (req, res) => {
    ...
    if (count && count >= 1 && count <= 10) {
        ...
        saveConfig();
        if (global.io) global.io.emit('config_updated', { activeTablesCount, pricePerFrame, appTheme });
...
app.post('/api/admin/config/theme', verifyAdminToken, (req, res) => {
    const { theme } = req.body;
    const allowedThemes = ['emerald', 'ocean', 'gold', 'midnight', 'triangle', 'waves', 'grid', 'carbon', 'city', 'abstract'];
    if (theme && allowedThemes.includes(theme)) {
        appTheme = theme;
        saveConfig();
        if (global.io) {
            console.log(`[CONFIG] Emitting theme update: ${appTheme}`);
            global.io.emit('config_updated', { activeTablesCount, pricePerFrame, appTheme });
        }
        res.json({ success: true, appTheme });
    } else {
